...
public class TestFairSchedulerSzw extends FairSchedulerTestBase {
  private final static String ALLOC_FILE =
          new File("F:\\test-queues").getAbsolutePath();
  ...
  public void testSimpleFairShareCalculation() throws IOException {
    ...
    for (FSLeafQueue p : queues) { //3414
      System.out.println(p.getQueueName() + ":" + p.getFairShare().getMemory() + "," + p.getMetrics().getFairShareMB() +
              "," + p.getSteadyFairShare().getMemory() + "," + p.getMetrics().getSteadyFairShareMB() +
              "," + p.getMetrics().getAllocatedMB() + "," + p.getMetrics().getAvailableMB());
    }
  }
  ...
  public void testFairShareWithMaxResources() throws IOException {
    ...
    System.out.println(conf.get(YarnConfiguration.RM_SCHEDULER_MINIMUM_ALLOCATION_MB));
...
//    createSchedulingRequest(1 * 1024, "root.default", "user1");
    ...
    queue = scheduler.getQueueManager().getLeafQueue(
        "queueB", false);
    System.out.println( "queueB : " + queue.getFairShare().getMemory() +
            ", weight : " + queue.getWeights().getWeight(ResourceType.MEMORY) +
            ", steadyShare : " + queue.getSteadyFairShare() +
            ", demand : " + queue.getDemand() +
            ", max/min : " + queue.getMaxShare() + "/" + queue.getMinShare());
    queue = scheduler.getQueueManager().getLeafQueue(
            "default", false);
    System.out.println( "default : " + queue.getFairShare().getMemory() +
            ", weight : " + queue.getWeights().getWeight(ResourceType.MEMORY) +
            ", steadyShare : " + queue.getSteadyFairShare() +
            ", demand : " + queue.getDemand() +
            ", max/min : " + queue.getMaxShare() + "/" + queue.getMinShare());
  ...
  public void testFairShareWithZeroWeight() throws IOException {
    ...
    System.out.println("cluster : " + scheduler.getClusterResource().getMemory());

    FSLeafQueue queue = scheduler.getQueueManager().getLeafQueue(
        "queueA", false);
    // queueA's weight is 0.0, so its fair share should be 0.
    System.out.println("queueA fairShare : " + queue.getFairShare().getMemory() +
        ", steadyShare : " + queue.getSteadyFairShare().getMemory() +
        ", demand : " + queue.getDemand() +
        ", maxShare : " + queue.getMaxShare() + "," + queue.getMinShare());
  ...
  public void testFairShareWithZeroWeightNoneZeroMinRes() throws IOException {
    ...
    System.out.println("queueA fair share : " + queue.getFairShare().getMemory() +
        ", steady share : " + queue.getSteadyFairShare());
    // queueB's weight is 0.0 and minResources is 1,
    // so its fair share should be 1 (minShare).
    queue = scheduler.getQueueManager().getLeafQueue(
        "queueB", false);
    System.out.println("queueB fair share : " + queue.getFairShare().getMemory()+
            ", steady share : " + queue.getSteadyFairShare());

    queue = scheduler.getQueueManager().getLeafQueue(
            "default", false);
    System.out.println("default fair share : " + queue.getFairShare().getMemory()+
            ", steady share : " + queue.getSteadyFairShare());
  }
  ...
  public void testSchedulerRootQueueMetrics() throws Exception {
    ...
    scheduler.update();
    System.out.println("root : " + scheduler.rootMetrics.getReservedMB());
...
//    createSchedulingRequest(2048, "queue2", "user1", 1);    // 0 ????
    createSchedulingRequest(1024, "queue2", "user1", 1);
    scheduler.update();
    scheduler.handle(updateEvent);

    // Make sure reserved memory gets updated correctly
    System.out.println("root reserved : " + scheduler.rootMetrics.getReservedMB());
    ...
    System.out.println("root old : " + scheduler.rootMetrics.getReservedMB());

    // ... but it should disappear when we update the first node.
    scheduler.handle(updateEvent);
    System.out.println("root update : " + scheduler.rootMetrics.getReservedMB());
  }
  ...
  public void testAssignToQueue() throws Exception {
    ...
    FSLeafQueue queue2 = scheduler.assignToQueue(rmApp2, "notdefault", "asterix");
//    FSLeafQueue queue2 = scheduler.assignToQueue(rmApp2, "notdefault", "obelix");
    ...
    System.out.println(rmApp1.getQueue() +  "," + queue1.getName());
    System.out.println("root.asterix" +  "," + rmApp1.getQueue());
    System.out.println(rmApp2.getQueue() +  "," + queue2.getName());
    System.out.println("root.notdefault" +  "," + rmApp2.getQueue());
  }
  ...
  public void testQueuePlacementWithPolicy() throws Exception {
    ...
    rules.add(new Default().initialize(true, null));
  ...
  public void testFairShareWithMinAlloc() throws Exception {
    ...
    for (FSLeafQueue p : queues) {
      if (p.getName().equals("root.queueA")) {
        System.out.println("queueA : " + p.getFairShare().getMemory());
      }
      else if (p.getName().equals("root.queueB")) {
        System.out.println("queueB : " + p.getFairShare().getMemory());
      }
    }
  }
...
//    System.out.println(conf.get(FairSchedulerConfiguration.PREEMPTION));
//    System.out.println(conf.get(FairSchedulerConfiguration.PREEMPTION_INTERVAL));
    ...
    System.out.println("app1 live containers : " + scheduler.getSchedulerApp(app1).getLiveContainers().size());
    System.out.println("app2 live containers : " + scheduler.getSchedulerApp(app2).getLiveContainers().size());
    System.out.println("app3 live containers : " + scheduler.getSchedulerApp(app3).getLiveContainers().size());
    System.out.println("app4 live containers : " + scheduler.getSchedulerApp(app4).getLiveContainers().size());
    ...
    System.out.println(root.getMinSharePreemptionTimeout());
...
//    scheduler.preemptResources(Resources.createResource(2 * 1024));
    System.out.println("=======================preemptResources======================");
    System.out.println("app1 live containers : " + scheduler.getSchedulerApp(app1).getLiveContainers().size());
    System.out.println("app3 live containers : " + scheduler.getSchedulerApp(app3).getLiveContainers().size());
    ...
    System.out.println("App2 should have container to be preempted" +
            scheduler.getSchedulerApp(app2).getLiveContainers() + " , " +
            scheduler.getSchedulerApp(app2).getPreemptionContainers());
    System.out.println("App4 should have container to be preempted" +
            scheduler.getSchedulerApp(app2).getLiveContainers() + " , " +
            scheduler.getSchedulerApp(app2).getPreemptionContainers());
    ...
    // now live containers is 1
    System.out.println("after preemptResources app2 live containers " + scheduler.getSchedulerApp(app2).getLiveContainers().size());
    System.out.println("after preemptResources app4 live containers " + scheduler.getSchedulerApp(app4).getLiveContainers().size());
    ...
    System.out.println("Containers with priority=4 in app2 and app4 should be " +
    ...
    scheduler.preemptResources(Resources.createResource(2 * 1024));
    System.out.println("App1 should have no container to be preempted" +
        scheduler.getSchedulerApp(app1).getPreemptionContainers().isEmpty());
    System.out.println("App2 should have no container to be preempted" +
        scheduler.getSchedulerApp(app2).getPreemptionContainers().isEmpty());
    System.out.println("App3 should have no container to be preempted" +
        scheduler.getSchedulerApp(app3).getPreemptionContainers().isEmpty());
    System.out.println("App4 should have no container to be preempted" +
  ...
  public void testPreemptionIsNotDelayedToNextRound() throws Exception {
    ...
    FSParentQueue root = scheduler.getQueueManager().getRootQueue();
    System.out.println(root.getMinSharePreemptionTimeout());
    System.out.println(root.getFairSharePreemptionTimeout());
    FSLeafQueue queue1 = scheduler.getQueueManager().getLeafQueue("queueA.queueA2", false);
    System.out.println(queue1.getMinSharePreemptionTimeout());
    System.out.println(queue1.getFairSharePreemptionTimeout());

    scheduler.update();
    Resource toPreempt = scheduler.resToPreempt(scheduler.getQueueManager()
        .getLeafQueue("queueA.queueA2", false), clock.getTime());
//    System.out.println("queueA1 fairShare and steadyFairShare : " +
//            scheduler.getQueueManager().getLeafQueue("queueA.queueA1", false).getFairShare() + "," +
//            scheduler.getQueueManager().getLeafQueue("queueA.queueA1", false).getSteadyFairShare());
    System.out.println("queueA2 fairShare and steadyFairShare : " +
            scheduler.getQueueManager().getLeafQueue("queueA.queueA2", false).getFairShare() + "," +
            scheduler.getQueueManager().getLeafQueue("queueA.queueA2", false).getSteadyFairShare());
//    System.out.println("queueB fairShare and steadyFairShare : " +
//            scheduler.getQueueManager().getLeafQueue("queueB", false).getFairShare() + "," +
//            scheduler.getQueueManager().getLeafQueue("queueB", false).getSteadyFairShare());
//    System.out.println("default fairShare and steadyFairShare : " +
//            scheduler.getQueueManager().getLeafQueue("default", false).getFairShare() + "," +
//            scheduler.getQueueManager().getLeafQueue("default", false).getSteadyFairShare());
    // toPreempt.getMemory is 3277
    System.out.println("queueA2 should preempt memory : " + toPreempt.getMemory());

    // verify if the 3 containers required by queueA2 are preempted in the same
    // round
    scheduler.preemptResources(toPreempt);
    System.out.println("app1 should preemptionContainers : " + scheduler.getSchedulerApp(app1).getPreemptionContainers()
        .size());
    // 并没有kill掉要preempt
    System.out.println("app1 live containers " + scheduler.getSchedulerApp(app1).getLiveContainers());
  }
